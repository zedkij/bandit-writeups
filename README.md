...
**Concept:**
`strings` command only returning strings data type and filters characters that readable by human like alphabets and numbers for example instead of binary data.
...
**Concept:**
SSH can execute a single remote command without opening a full interactive shell (e.g. `ssh user@host command`). This matters here because the interactive shell gets killed right after login — running the command inline avoids needincatg that shell at all.
...
**Method:**
Go to the cron directory, list the files there, read bandit22's cron job entry, then read the script it runs, sread the file mentioned in that script, and read the file the password of next level redirected into.
...
**Concept:**
Exploiting loop hole in the system
Echoing the phrase give a predictable result
it doesnt verify further, one creditential enough
...
**Concept:**
it has loose permission
it doesn't verify further
